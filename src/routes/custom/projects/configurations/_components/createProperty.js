    ...
    const onSubmit = () => {
        ...
        if(nature.value == 'PROJECT') {
            if(selectedCells.length > 0) {
                ...
                data.object_reference = "("+selectedCells[0].row+", "+selectedCells[0].column+", "+selectedCells[0].position+")";
            } else {
                    ...
                    objectItemParsed && (
                        objectItemParsed.type == 'TABLE' && (
                            <UpdateComplexTable 
                                ...
                                onSelected={(cell) => {
                                    let selected = selectedCells.filter(sc => sc.row == cell.row && sc.column == cell.column && sc.position == cell.position);
                                    if(selected.length > 0) {
                                        setSelectecCells([...selectedCells.filter(sc => sc.row != selected[0].row || sc.column != selected[0].column || sc.position != selected[0].position)]);
                                    } else {
