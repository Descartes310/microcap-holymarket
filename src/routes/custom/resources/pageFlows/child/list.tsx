...
const List = (props) => {
    ...
    const getPages = () => {
        ...
        PageFlowService.getChild(props.match.params.id)
        .then(response => {
            setPages(response);
        })
        .catch(err => {
    ...
    return (
        <>
            <PageTitleBar
                title={"Liste des pages: "+parent?.label}
                onBackClick={() => {
            ...
            <CustomList
                ...
                onAddClick={() => props.history.push(joinUrlWithParamsId(RESOURCES.PAGE_FLOWS.CHILD.CREATE, props.match.params.id))}
                renderItem={list => (
                    <>
                        {list && list.length === 0 ? (
                                        ...
                                        {list && list.map((item, key) => (
                                            <tr key={key} className="cursor-pointer">
                                                ...
                                                <td>
                                                    <div className="media">
                                                        <div className="media-body pt-10">
                                                            <p className="m-0 text-dark">{item.link}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
