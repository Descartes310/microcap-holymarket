...
import AddSynchronisation from '../components/addSynchronisation';
...
const Synchronisation = (props) => {
    ...
    const getConsolidations = () => {
        ...
        AccountService.getSynchronisations(account.reference).then(response => {
            setAccounts(response.map(c => c.synchronised));
        }).catch((err) => {
            console.log(err);
        })
        .finally(() => {
    ...
    const deleteAccount = () => {        
        if(!selectedAccount) {
            NotificationManager.error('Selectionnez le compte à rétirer')
            return;
        }
        props.setRequestGlobalAction(true);
        AccountService.deleteSynchronisation(account?.reference, selectedAccount?.reference)
        .then((response) => {
    ...
    return (
        <>
            <PageTitleBar
                title={"Synchronisations du compte"}
            />
            <CustomList
                list={accounts}
                loading={false}
            ...
            <ConfirmBox
                ...
                message={"Souhaitez-vous rétirer ce compte de la synchronisation?"}
            />
            { account && (
                <AddSynchronisation
                    show={showAddBox}
...
export default connect(mapStateToProps, { setRequestGlobalAction })(withRouter(Synchronisation));