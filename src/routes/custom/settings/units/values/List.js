...
import { getUnits, setRequestGlobalAction, createUnit, getUnitTypes } from "Actions";
...
class List extends Component {
    ...
    loadType() {
        ...
        ).catch(err => this.setState({ types: [] }))
    }

    createNewUnit() {
        this.setState({ show: false })
        this.props.setRequestGlobalAction(true)
        createUnit({ name: this.state.name, code: this.state.code, unit_type_id: this.state.type.id }).then(data => {
            this.loadData();
        }).catch(err =>
            NotificationManager.error("L'unité existe déjà")
        ).finally(() => {
        ...
        return (
            <>
                <CustomList
                    ...
                    itemsFoundText={n => `${n} d'unités trouvées`}
                    /*addPermissions={{
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-12 col-sm-12">
                                <FormControl fullWidth>
                                    <InputLabel className="text-left pl-2" htmlFor="unitType">
                                        Type d'unité
                                    </InputLabel>
