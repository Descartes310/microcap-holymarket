...
const Create = (props) => {
    ...
    const onSubmit = () => {
        ...
        BankService.createCashdesk(data).then(() => {
            NotificationManager.success("La caisse a été créé avec succès");
            props.history.push(BANK.PARTY.CASHDESK.LIST);
        }).catch((err) => {
            console.log(err);
            NotificationManager.error("Une erreur est survenu lors de la caisse");
        }).finally(() => {
            props.setRequestGlobalAction(false);
        })
    }
