...
const Create = (props) => {
    ...
    const onSubmit = () => {
        ...
        BrokerService.createCashdesk(data).then(() => {
            NotificationManager.success("La caisse a été créée avec succès");
            props.history.push(BROKER.CASHDESK.LIST);
        }).catch((err) => {
            console.log(err);
            NotificationManager.error("Une erreur est survenu lors de la création de la caisse");
        }).finally(() => {
