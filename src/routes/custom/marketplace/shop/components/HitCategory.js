...
class HitCategory extends Component {
	state = {
		loading: false
	}
		...
		return (
			<RctCard colClasses="d-flex col-md-3 col-sm-6 mb-0 flex-column justify-content-between overflow-hidden cursor-pointer">
				<div className="overlay-wrap overflow-hidden" onClick={() => {
					this.props.history.push(joinUrlWithParams(MARKETPLACE.SHOP.HOLYMARKET.MARKETS, [{param: 'categoryReference', value: category.reference}]));
				}}>
				...
				>
					<h4 className="text-dark">{textTruncate(category.label, 25)}</h4>
					<p className="mb-5 text-muted font-xs">
						{textTruncate(category.label, 50)}
					</p>
				</div>
			</RctCard>
		)
...
export default connect(mapStateToProps, {})(withRouter(HitCategory));