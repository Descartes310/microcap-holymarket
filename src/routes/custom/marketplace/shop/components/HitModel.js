		...
		return (
			<RctCard colClasses="d-flex col-md-3 col-sm-6 mb-0 flex-column justify-content-between overflow-hidden cursor-pointer">
				<div className="overlay-wrap overflow-hidden" onClick={() => {
					this.props.history.push(joinUrlWithParams(MARKETPLACE.SHOP_PRODUCTS, [{param: 'reference', value: product.reference}])+`${(match.params.categoryReference && match.params.marketReference) ? '?category='+match.params.categoryReference+'&market='+match.params.marketReference : ''}`);
				}}>
